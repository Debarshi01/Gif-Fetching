...
function App(){
    ...
    const fetchSearchGif  = async(title)=>{
        const response = await axios.get(`https://api.giphy.com/v1/gifs/search/?api_key=${api}&limit=25&q=${title}`)
    ...
    const AddFav = (id)=>{
        const favGifstoAdd = trendGif.find((gif)=> gif.id === id) || searchGif.find((gif)=> gif.id === id)
    ...
    <div>
        ...
        <>
          ...
          <br/>
          <br/>
          <GifList Gifs={searchGif} Addfav={AddFav} onClick={handleClick} />
        </>
      )}
       <br/>
        <br/>
